      ...
      <PageHeader
        ...
        actions={puedeVerManuales ? (
          <a
            ...
            className="px-3 py-1.5 rounded-lg border text-xs font-medium transition-all duration-200 inline-flex items-center gap-1.5"
            style={{
          ...
          >
            ...
            <span>Ver manuales de video del sistema</span>
            <FaExternalLinkAlt className="w-2.5 h-2.5 opacity-70" />
          </a>
        ) : null}
      />

      {/* Header con avatar y acciones - Clean Card UI */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="flex-1 text-center sm:text-left min-w-0">
            ...
            <p className="text-sm text-gray-500">@{perfil?.username}</p>
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mt-1">
              <span 
                className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold text-white"
                style={{ backgroundColor: 'var(--color-primary, #9F2241)' }}
              >
                <FaIdBadge className="w-3 h-3 mr-1" />
                {rolDisplay}
              </span>
          ...
          <div className="flex items-center">
            <button
              onClick={() => setShowLogoutConfirm(true)}
              disabled={loggingOut}
              className="px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 inline-flex items-center gap-2 border"
              style={{
