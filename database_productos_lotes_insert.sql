...
-- NOTA: Los productos se insertan primero, luego los lotes
...
INSERT INTO productos (codigo_barras, nombre, presentacion, concentracion, categoria, unidad_medida, stock_minimo, activo, created_at, updated_at)
VALUES
-- Fila 1-10
...
-- Productos adicionales de la imagen (medicamentos específicos)
('AMPMAB', 'AMPOLLETA BARRIO DE CABALERO, CONTIENE MEDICAMENTOS ANTÍGENOS EN SOLUCIÓN', 'AMPOLLETA', NULL, 'medicamento', 'ampolleta', 10, true, NOW(), NOW()),
('DIACEF', 'DIACEFICA, SÓDICO DICLUMINA CON LIDOSINA', 'AMPOLLETAS', NULL, 'medicamento', 'ampolleta', 10, true, NOW(), NOW()),
...
-- Continuar con más productos según imagen...
...
-- NOTA: Solo se insertan lotes con cantidad > 0 (constraint lotes_cantidad_inicial_check)
-- ══════════════════════════════════════════════════════════════════════════════

-- Nota: Se necesita el producto_id real. Usamos subquery para obtenerlo.
-- Los lotes se insertan después de que existan los productos.
...
SELECT 
    ...
    v.numero_contrato,
    NULL as marca,
    1 as centro_id,  -- Almacén Central
    true as activo,
    NOW() as created_at,
    NOW() as updated_at
FROM (VALUES
    -- (codigo_barras, numero_lote, cantidad, fecha_caducidad, numero_contrato)
    ...
    -- Medicamentos con C (solo con stock)
...
) AS v(codigo_barras, numero_lote, cantidad, fecha_caducidad, numero_contrato)
JOIN productos p ON p.codigo_barras = v.codigo_barras
