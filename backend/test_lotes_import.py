"""
Script para probar la importación de lotes con el archivo real.
"""
import os
import sys
import django
from pathlib import Path

# Setup Django
sys.path.insert(0, str(Path(__file__).parent))
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings')
django.setup()

from django.contrib.auth import get_user_model
from core.utils.excel_importer import importar_lotes_desde_excel
from django.core.files.uploadedfile import InMemoryUploadedFile
from io import BytesIO

User = get_user_model()

# Configuración
ARCHIVO_LOTES = r"C:\Users\zarag\Downloads\REVISAR\lotes_2025-12-15.xlsx"

def test_lotes_import():
    """Prueba la importación de lotes con archivo real."""
    
    print("=" * 70)
    print("TEST DE IMPORTACIÓN DE LOTES")
    print("=" * 70)
    
    # 1. Verificar archivo
    if not os.path.exists(ARCHIVO_LOTES):
        print(f"\n❌ ERROR: No se encuentra el archivo: {ARCHIVO_LOTES}")
        return
    
    print(f"\n✓ Archivo encontrado: {ARCHIVO_LOTES}")
    file_size = os.path.getsize(ARCHIVO_LOTES)
    print(f"  Tamaño: {file_size / 1024:.2f} KB")
    
    # 2. Obtener usuario farmacia
    try:
        user = User.objects.filter(rol='farmacia').first()
        if not user:
            print("\n❌ ERROR: No hay usuario con rol 'farmacia'")
            return
        
        print(f"✓ Usuario farmacia: {user.email}")
    except Exception as e:
        print(f"\n❌ ERROR al buscar usuario: {e}")
        return
    
    # 3. Abrir archivo y ejecutar importación
    try:
        print(f"\n🔄 Importando lotes desde archivo...")
        
        with open(ARCHIVO_LOTES, 'rb') as file:
            # Crear objeto de archivo simulado
            content = file.read()
            file_obj = InMemoryUploadedFile(
                file=BytesIO(content),
                field_name='file',
                name=os.path.basename(ARCHIVO_LOTES),
                content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                size=len(content),
                charset=None
            )
            
            # Ejecutar importación (SIN centro_id para que use la columna Centro)
            resultado = importar_lotes_desde_excel(file_obj, user, centro_id=None)
            
            print(f"\n📊 RESULTADO:")
            print(f"   Exitosa: {resultado.get('exitosa', False)}")
            print(f"   Total procesados: {resultado.get('total_registros', 0)}")
            print(f"   Exitosos: {resultado.get('registros_exitosos', 0)}")
            print(f"   Fallidos: {resultado.get('registros_fallidos', 0)}")
            
            if resultado.get('errores'):
                print(f"\n⚠️  ERRORES ENCONTRADOS ({len(resultado['errores'])}):")
                for i, err in enumerate(resultado['errores'][:10], 1):
                    print(f"   {i}. {err}")
                if len(resultado['errores']) > 10:
                    print(f"   ... y {len(resultado['errores']) - 10} más")
            
            resumen = resultado.get('resumen', {})
            if resumen:
                print(f"\n📋 RESUMEN:")
                print(f"   Creados: {resumen.get('creados', resultado.get('creados', 0))}")
                print(f"   Actualizados: {resumen.get('actualizados', 0)}")
                print(f"   Omitidos: {resumen.get('omitidos', 0)}")
            
            if resultado.get('exitosa'):
                print(f"\n✅ IMPORTACIÓN EXITOSA!")
            elif resultado.get('registros_exitosos', 0) > 0:
                print(f"\n⚠️  IMPORTACIÓN PARCIAL")
            else:
                print(f"\n❌ IMPORTACIÓN FALLIDA")
                
    except Exception as e:
        print(f"\n❌ ERROR DURANTE IMPORTACIÓN:")
        print(f"   Tipo: {type(e).__name__}")
        print(f"   Mensaje: {str(e)}")
        import traceback
        print(f"\n   Traceback:")
        traceback.print_exc()
    
    print("\n" + "=" * 70)


if __name__ == '__main__':
    test_lotes_import()
