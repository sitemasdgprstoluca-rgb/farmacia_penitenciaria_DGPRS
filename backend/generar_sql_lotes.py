...
print(f'Total lotes a insertar: {len(df)}')
print(f'\nPrimeras 5 filas procesadas:')
print(df[['Clave', 'Nombre Producto', 'Número Lote', 'Cantidad Inicial', 'Cantidad Actual']].head())

# Generar SQL
sql_output = []
sql_output.append('-- Script de inserción de lotes para Supabase')
sql_output.append('-- Generado: ' + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
sql_output.append('-- Total de lotes: ' + str(len(df)))
sql_output.append('')
...
sql_output.append('')
sql_output.append('-- =============================================================================')
sql_output.append('-- INSERCIÓN DE LOTES')
sql_output.append('-- =============================================================================')
sql_output.append('')
...
for idx, row in df.iterrows():
    try:
        ...
        # Generar INSERT
        sql_output.append(f'-- Lote: {numero_lote} - Producto: {clave}')
...
# Guardar archivo
output_file = 'lotes_supabase_insert.sql'
